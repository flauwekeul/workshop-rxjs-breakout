...
export const drawCircle = (
  canvasContext: CanvasRenderingContext2D,
  { x, y, radius, color }: RenderedCircle
): void => {
  // beginPath() is needed to clear any previously drawn paths
  canvasContext.beginPath();
...
export const drawText = (
  canvasContext: CanvasRenderingContext2D,
  {
    ...
    size = 20,
    textAlign = "left",
  }: Optional<Text, "color" | "size" | "textAlign">
): void => {
...
export const hasBallPassedPaddle = (ballY: number, paddle: Position) =>
  ballY > paddle.y + PADDLE_HEIGHT;

export const getBrickCollision = (
  ball: Circle,
  bricks: Brick[]
): BrickCollision | undefined => {
  for (let i = 0; i < bricks.length; i++) {
    ...
    if (
      distX > halfBrickWidth + ball.radius ||
      distY > halfBrickHeight + ball.radius
    ) {
      continue;
    }
...
export const brickBuilder = (canvas: HTMLCanvasElement) => {
  ...
  const brickWidth = Math.floor(
    (canvas.width - 2 * BRICKS_MARGIN) / BRICKS_PER_ROW
  );
...
export const formatNumber = (score) =>
  new Intl.NumberFormat("en-GB").format(score);
...
export const getPaddleBounceDirection = lerp(
  FAR_LEFT_BOUNCE_DIRECTION,
  FAR_RIGHT_BOUNCE_DIRECTION
);
